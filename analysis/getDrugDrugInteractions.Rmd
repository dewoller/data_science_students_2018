...
After conversion, we wrote a regular expression script in Perl to only extract the drugs name. Each drugs were having interactions with some other multiple drugs. For example, Abacavir	-> RIBAVIRIN ; Abacavir	-> TIPRANAVIR; Abacavir -> ORLISTAT. Also Tipranavir -> CLARITHROMYCIN ; Tipranavir ->	RIFABUTIN ;Tipranavir	-> RIFAMPICIN.
...
We need to bind these 2 rows, so that these interaction can be removed for further analysis. The code to do this is just below:
